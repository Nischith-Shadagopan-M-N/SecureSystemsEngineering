...
#define public_mAIN_ARENA_INFo dlmain_arena_info;;
#else /* USE_DL_PREFIX */
#ifdef _LIBC
...
#define public_mAIN_ARENA_INFo __malloc_main_arena_info;;
...
#define public_mAIN_ARENA_INFo  main_arena_info
#endif /* _LIBC */
#endif /* USE_DL_PREFIX */
...
#if __STD_C
Void_t  public_mAIN_ARENA_INFo(int);
#else
Void_t  public_mAIN_ARENA_INFo();
#endif
...
void print_bins(mchunkptr start, mchunkptr ptr){
	if (ptr == 0) return;
	if (start == ptr) return;
	if (ptr->fd == ptr->bk) return;
	if (start == 0) start = ptr; 


	printf("chunk %08x\t", ptr);
	printf("pointer %08x\t", ((int)ptr) + 0x10);
	printf("size        %03d\t", ptr->size);
	printf("forward ptr %08x\t", ptr->fd);
	printf("back ptr    %08x\n", ptr->bk);



	print_bins(start, (mchunkptr) ptr->fd);
}


void print_fastbins(mchunkptr start, mchunkptr ptr){
	if (ptr == 0) return;
	if (start == ptr) return;
	if (start == 0) start = ptr; 
	...
	print_fastbins(start, (mchunkptr) ptr->fd);
}

void
public_mAIN_ARENA_INFo(int what)
{
	int i;
	printf("In main_arena_info\n");
	printf("Number of FastBins : %d\n", NFASTBINS);
	printf("Number of Bins : %d\n", NBINS * 2);

	if (what == 1){
		/* print the fastbins */
		for(i=0; i<NFASTBINS; ++i){
			printf("%d Fastbin : %x\n", i, main_arena.fastbins[i]);
			print_fastbins(0, main_arena.fastbins[i]);
		}
	}

#if 1
	if (what == 2){
		for(i=0; i<NBINS*2; i+=2){
			printf("%d bin : %x\n", i, main_arena.bins[i]);
			print_bins(0, main_arena.bins[i]);
		}
	}
#endif
...
weak_alias (__malloc_main_arena_info, main_arena_info)
